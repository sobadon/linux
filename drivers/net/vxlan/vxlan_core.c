		    ...
		    __be32 default_vni, struct vxlan_rdst *rdst, bool did_rsc)
{
	...
	if (addr_family == AF_INET) {
		...
		// need to fragment や packet too big の ICMP エラーが返される（返されててしまう）。
		// ignore-df するとき、PMTU の結果を返答する必要がないので、強制的に false で返さないようにする。
		// patch 前は、以下のような MTU 不一致のメッセージを返答していた。
		...
		// vxlan_xmit() のコメントでは、inner の DF を outer に継承するというのを無視する形
		// 参考：
		// - OpenVPN で outer IPv6 パケットの ignore_df: https://github.com/OpenVPN/ovpn-dco/commit/3ba6c07ababd4d491c77f5327f8105bead6ddccc
		// メモ：
		// - `skb_scrub_packet()` までは `ignore_df = 1` がいるものの、
		// - `pskb_expand_head()` になると消えている。
		skb->ignore_df = 1;
